...
\begin{description}
  ...
  \item [Converted] We have imported the chapter from docx to \LaTex -
    we need to typeset the chapter. This includes first running 'make
    patch' the modify 'typeset.tex' with new typesetting and
    potentially with the contents of typeset.reject. When theis is
    done save typeset.tex and run 'make diff' Then run 'make pdf'
    inspect the pdf and commit 'typeset.patch' but not the pdf.
