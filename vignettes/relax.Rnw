...
\title{\pkg{relax}: An \proglang{R} package for authoring reports}

\Plaintitle{relax: An R package for authoring reports}

\Shorttitle{\pkg{relax}: An R package for authoring reports}
...
%\VignetteIndexEntry{relax: An R package for authoring reports}
%\VignetteDepends{relax}
%\VignetteKeywords{FIXME}
%\VignettePackage{relax}
...
The \pkg{relax} package is available from GitHub at
\mbox{\url{https://github.com/stewid/relax}} and is loaded in
